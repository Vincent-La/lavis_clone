...
#SBATCH --job-name=img_submodule_w4_a32                       # sets the job name
#SBATCH --output=img_submodule_w4_a32.%j                    # indicates a file to redirect STDOUT to; %j is the jobid. If set, must be set to a file instead of a directory or else submission will fail.
#SBATCH --error=img_submodule_w4_a32.%j                     # indicates a file to redirect STDERR to; %j is the jobid. If set, must be set to a file instead of a directory or else submission will fail.
...
python -m torch.distributed.run \
       ...
       --img-submodule-FF-weight_bits 4 \
