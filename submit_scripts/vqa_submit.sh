...
#SBATCH --job-name=blip2_vqa_baseline                     # sets the job name
#SBATCH --output=blip2_vqa_baseline.%j                    # indicates a file to redirect STDOUT to; %j is the jobid. If set, must be set to a file instead of a directory or else submission will fail.
#SBATCH --error=blip2_vqa_baseline.%j                     # indicates a file to redirect STDERR to; %j is the jobid. If set, must be set to a file instead of a directory or else submission will fail.
...
python -m torch.distributed.run \
       ...
       --cfg-path vqav2_zeroshot_flant5xl_eval.yaml
